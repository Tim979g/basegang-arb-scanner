...
const V3_QUOTER_ABI = ["function quoteExactInputSingle((address tokenIn,address tokenOut,uint256 amountIn,uint24 fee,uint160 sqrtPriceLimitX96) params) returns (uint256 amountOut,uint160 sqrtPriceX96After,uint32 initializedTicksCrossed,uint256 gasEstimate)"];
...
export class LiveQuoteVerifier {
  ...
  async poolQuote(pool, tokenIn, amountIn) {
    const tokenOut = pool.base.address.toLowerCase() === tokenIn.address.toLowerCase() ? pool.quote : pool.base;
    if (pool.dex === "aerodrome") return new Contract(pool.pairAddress, POOL_ABI, this.provider).getAmountOut(amountIn, tokenIn.address, { blockTag: "pending" });
    if (pool.dex === "uniswap") {
      const fee = await new Contract(pool.pairAddress, V3_POOL_ABI, this.provider).fee({ blockTag: "pending" });
      const quote = await new Contract(UNISWAP_V3_QUOTER, V3_QUOTER_ABI, this.provider).quoteExactInputSingle.staticCall([tokenIn.address, tokenOut.address, amountIn, fee, 0], { blockTag: "pending" });
      return quote.amountOut;
    }
    throw new Error(`Unsupported live-quote DEX: ${pool.dex}`);
  }

  async verifyDirect(item, tradeSizeUsd, costs) {
    if (![item.buy.dex, item.sell.dex].every((dex) => dex === "aerodrome" || dex === "uniswap")) return { status: "unsupported_dex" };
  ...
  async verifyTriangle(item, tradeSizeUsd, tokenBySymbol, costs) {
    if (!item.edges.every((edge) => edge.dex === "aerodrome" || edge.dex === "uniswap")) return { status: "unsupported_dex" };
    try {
